...
namespace TechnikumDirekt.IntegrationTests
{
    ...
    { 
        ...
        public async Task PostTransferParcelFromPartner_validParcelValidTrackingId_OkValidTrackingId()
        {
            ...
            var response = await Client.PostAsync(Client.BaseAddress.AbsoluteUri + $"/parcel/{ValidTrackingId}", parcelContent);
        ...
        public async Task PostTransferParcelFromPartner_validParcelTrackingIdAlreadyTaken_BadRequest()
        {
            ...
            // Act
            await Client.PostAsync(Client.BaseAddress.AbsoluteUri + "/warehouse", content);
            await Client.PostAsync(Client.BaseAddress.AbsoluteUri + $"/parcel/{ValidTrackingId}", parcelContent);
            var response = await Client.PostAsync(Client.BaseAddress.AbsoluteUri + $"/parcel/{ValidTrackingId}", parcelContent);
