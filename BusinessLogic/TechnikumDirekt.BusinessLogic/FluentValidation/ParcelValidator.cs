...
namespace TechnikumDirekt.BusinessLogic.FluentValidation
{
    public class ParcelValidator : AbstractValidator<Parcel>
    {
        public ParcelValidator()
        {
            ...
            RuleSet("trackingId", () => { RuleFor(p => p.TrackingId).Matches(@"^[A-Z0-9]{9}$"); });
            ...
            RuleFor(p => p.VisitedHops).NotNull().ForEach(vh => { vh.SetValidator(new HopArrivalValidator()); });
            RuleFor(p => p.FutureHops).NotNull().ForEach(fh => { fh.SetValidator(new HopArrivalValidator()); });
        }
    }
}